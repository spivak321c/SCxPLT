...
export default function FeastForTheEyes() {
  ...
  return (
    <section className="py-24 bg-[#f8f9fa] relative overflow-hidden" id="feast-section">
      ...
      <div 
        className="absolute top-[30%] left-[10%] w-96 h-96 pointer-events-none" 
        style={{
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10">
        ...
        {/* Dynamic Portfolio Tabs Selector & Case Study Grid Container (Desktop only) */}
        <div className="hidden lg:block" id="portfolio-desktop-showcase">
              ...
              >
                ...
                <div className="flex items-center justify-between border-b-2 border-black pb-3 mb-4">
                  ...
                  <span className="text-[9px] font-mono text-black font-black uppercase tracking-wider">
                    {currentStudy.client.toLowerCase().replace(" ", "")}-draft.scvlpt.studio
                  </span>
                  <div className="w-4" />
                </div>
                  ...
                  <div className="relative h-48 md:h-full min-h-[200px] rounded-none overflow-hidden border-2 border-black group" id="portfolio-mock-photo">
                    ...
                    <div className="absolute bottom-3 left-3 right-3 bg-white border-2 border-black p-2 rounded-none flex items-center gap-2 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                      ...
                      <div className="text-left">
                        ...
                        <span className="text-[8px] font-sans text-neutral-600 font-semibold block">"The booking rate has doubled."</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="block lg:hidden space-y-4" id="portfolio-mobile-showcase">
          {CASE_STUDIES.map((study) => {
            const isActive = study.id === activeStudy;
            return (
              <div 
                key={study.id} 
                className="border-4 border-black bg-white overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
                id={`portfolio-mobile-accordion-wrapper-${study.id}`}
              >
                {/* Accordion Header Button */}
                <button
                  type="button"
                  onClick={() => setActiveStudy(isActive ? "" : study.id)}
                  ...
                  id={`portfolio-mobile-accordion-header-${study.id}`}
                >
                  <span className="font-sans font-black text-xs sm:text-sm uppercase tracking-wider">{study.client}</span>
                ...
                {isActive && (
                  <div className="border-t-4 border-black bg-neutral-50 p-4 space-y-6 text-left" id={`portfolio-mobile-accordion-content-${study.id}`}>
                    
                    {/* Category tag & Client name */}
                    <div className="flex items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded-none text-[9px] font-mono uppercase bg-white border-2 border-black text-purple-600 font-black">
                        {study.category}
                      </span>
                      <span className="text-black font-mono text-[9px]">•</span>
                      <span className="text-black font-mono text-[9px] font-black uppercase">{study.client}</span>
                    </div>

                    {/* Title */}
                    <h3 className="text-lg sm:text-xl font-sans font-black text-black tracking-tighter uppercase leading-tight">
                      {study.title}
                    </h3>
                    ...
                    {/* Metrics Display */}
                    <div className="p-4 rounded-none border-4 border-black bg-white flex items-center gap-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                      <div className="p-2.5 rounded-none bg-neutral-100 border-2 border-black text-black shrink-0">
                        <TrendingUp className="w-5 h-5" />
                      </div>
                      <div>
                        <span className="text-lg sm:text-xl font-mono font-black text-black block leading-none">{study.metrics}</span>
                        <p className="text-[9px] font-mono text-neutral-500 uppercase tracking-wider font-black mt-1">
                          {study.metricLabel}
                        </p>
                      </div>
                    </div>

                    {/* Interactive Device/Browser Mockup */}
                    <div className="border-4 border-black bg-white p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] space-y-4">
                      {/* Browser address bar */}
                      <div className="flex items-center justify-between border-b-2 border-black pb-2">
                        <div className="flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full border border-black bg-black" />
                          <span className="w-2 h-2 rounded-full border border-black bg-black" />
                          <span className="w-2 h-2 rounded-full border border-black bg-black" />
                        </div>
                        <span className="text-[8px] font-mono text-black font-black uppercase tracking-wider">
                          {study.client.toLowerCase().replace(" ", "")}-draft.scvlpt.studio
                        </span>
                        <div className="w-4" />
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {/* Left brand mockup card */}
                        <div className="p-3.5 rounded-none bg-white border-2 border-black space-y-3 shadow-[2px_2px_0px_0px_#1a1a1a]">
                          <div className="flex items-center gap-1.5">
                            <span className="w-4.5 h-4.5 rounded-none bg-black text-white border border-black font-sans font-black text-[8px] flex items-center justify-center px-1">
                              {study.logoText}
                            </span>
                            <span className="text-[10px] font-sans font-black uppercase text-black">{study.client}</span>
                          </div>
                          <p className="text-xs font-sans font-black text-black leading-tight">
                            {study.mockupText}
                          </p>
                          <div className="space-y-1">
                            <span className="w-12 h-1 bg-neutral-200 border border-black rounded-none block" />
                            <span className="w-9 h-1 bg-neutral-200 border border-black rounded-none block" />
                          </div>
                        </div>

                        {/* Right image + Testimonial overlay */}
                        <div className="relative h-44 rounded-none overflow-hidden border-2 border-black">
                          <img
                            src={study.image}
                            alt={study.client}
                          ...
                          {/* Rating bubble */}
                          <div className="absolute bottom-2 left-2 right-2 bg-white border-2 border-black p-1.5 rounded-none flex items-center gap-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                            <img
                              src={study.image}
                              alt="avatar"
                              className="w-5 h-5 rounded-none object-cover border border-black"
                              referrerPolicy="no-referrer"
                            />
                            <div className="text-left leading-none">
                              <span className="text-[8px] font-sans font-black text-black block uppercase">Larisa G.</span>
                              <span className="text-[7px] font-sans text-neutral-600 font-semibold block mt-0.5">"The booking rate has doubled."</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="pt-4 border-t-2 border-black">
                      <button
                        onClick={() => setIsInquiryOpen(true)}
                        className="w-full py-2.5 px-4 rounded-none bg-black border-2 border-black text-white font-sans font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-[4px_4px_0px_0px_#7c3aed] cursor-pointer"
                      >
                        <span>Read case study & request draft</span>
                        <ArrowUpRight className="w-4 h-4 text-purple-400" />
                      </button>
